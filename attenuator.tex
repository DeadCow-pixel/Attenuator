...
Homodyne and double-homodyne (heterodyne) detection are standard coherent-detection schemes in continuous-variable quantum optics and quantum information~\cite{Leonhardt:bk:1997,Braunstein:rmp:2005,RevModPhys.84.621}. Homodyne detection measures a single field quadrature, whereas double-homodyne detection implements a joint Gaussian measurement of two conjugate quadratures. These schemes constitute the measurement stage in a broad class of continuous-variable quantum-communication protocols, including protocols for continuous-variable quantum key distribution (CV-QKD)~\cite{PhysRevLett.88.057902,Pirandola:20,Zhang:apr:2024}, quantum state tomography~\cite{d2003quantum, Lvovsky:rmp:2009} and quantum state preparation~\cite{Zhang_2019}.
...
Previous studies have established that receiver imbalance and other nonidealities can bias channel-parameter estimation, reduce the secret-key rate, and create security gaps when omitted from the measurement model~\cite{Pereira2021,PRXQuantum.3.010341,hajomer2025finite,Bartlett:25}.
...
We compare the two channels within the Gaussian-modulated coherent-state (GMCS) protocol~\cite{Grosshans:prl:2002} with reverse reconciliation in the asymptotic regime and evaluate the asymptotic secret fraction under collective Gaussian attacks~\cite{Navascues2006GaussianAttacks,GarciaPatron2006GaussianAttacks}. The GMCS protocol provides a natural setting for a direct covariance-matrix comparison of the two detector-channel realizations. In the security model adopted here, receiver asymmetry is treated as untrusted; accordingly, the environmental output of the selected realization is assigned to Eve. Finite-size effects, composable-security corrections, active detector attacks, and uncertainties in equipment calibration are outside the scope of the present work.
...
\begin{align}
    ...
    ,
\end{align}
where $\Sigma_{\ind{id}}^{(\ind{H})}$ is the covariance matrix of ideal measurement, $R(\phi)=\begin{pmatrix}\cos\phi&-\sin\phi\\ \sin\phi& \cos\phi\end{pmatrix}$ is the rotation matrix; $\sigma_N$ is the excess noise variance that accounts for asymmetry effects is given by
\begin{align}
  &
  \label{eq:sgm_N}
     \sigma_N=\sigma_x-1.
...
parameters and, for double-homodyne detection, also from the selected Gaussian seed.

After the deterministic invertible rescaling of the measurement
outcomes, the two realizations reproduce the same conditional
...
entropy $S_{A|B}$, are the same for the two realizations because the
detector imperfections enter the conditional state through the same
noisy measurement. The difference between the security estimates originates from the joint state of Alice and Bob. The additive-noise and attenuator channels generally produce different joint covariance matrices and, consequently, different joint entropies. In the untrusted-noise scenario considered here, the environment associated with the detector and channel is assigned to Eve. The corresponding Holevo information, $\chi_{EB}=S_{AB}-S_{A|B}$, therefore depends on the selected channel realization even when Bob's observed statistics remain unchanged.
...
pure-loss channel. Within the combined channel-detector model, this
realization rescales both the effective transmission and the
...
attenuator realizations are not uniformly ordered. At high channel
transmittance and for the parameters considered, the attenuator
realization gives a larger Holevo information and a lower asymptotic
secret fraction. At longer distances, however, the secret fraction remains positive up
to a greater channel length than in the additive-noise realization. Therefore, neither realization can be described as
uniformly more or less pessimistic. Their ordering is therefore not uniform over the parameter regimes
considered.
...
parameter $r$. The parameter $r$ labels the ideal pure Gaussian seed
measurement used in the decomposition of this fixed POVM into an ideal
measurement and a preceding Gaussian channel. The requirement that the excess-noise covariance matrix be positive
semidefinite restricts $r$ to the interval given by
Eq.~\eqref{eq:r1-r2}.

For the additive-noise realization, variation of $r$ redistributes the
measurement noise between the two conjugate quadratures according to
Eq.~\eqref{eq:sigma_N-sq-2}. For the attenuator realization, the same variation changes
...
attenuator realization, the environmental state approaches an
...
therefore understood as limiting realizations in which the attenuator
...
$r$ in qualitatively different ways for the two channel realizations.
For the parameter regimes considered, this dependence is concave for
the additive-noise realization and convex for the attenuator
realization. When the measurement noise is symmetric between the two
quadratures, both curves are symmetric about $r=0$. In this case, the unsqueezed seed, $r=0$, maximizes the Holevo
information for the additive-noise realization and minimizes it for the
attenuator realization.
...
obtained by varying $r$ over its admissible interval for each channel realization. For the near-ideal quadrature variances used in
Fig.~\ref{fig:K-vs-L-q-add}, the lower bounds of the two realizations nearly coincide
...
channel realization and the selected seed parameter.

These results clarify the role of the detector-channel representation
in the untrusted-noise model. Although the additive-noise and
attenuator descriptions reproduce the same receiver statistics after
outcome rescaling, they generally yield different joint states of Alice
and Bob and different outputs assigned to Eve. The
selected representation therefore introduces a physical assumption
about the coupling between the optical signal and the inaccessible
degrees of freedom.

A security estimate should not be based on a particular representation
solely because it gives a larger secret fraction. The selected
detector-channel model should either be justified from the physical
structure of the receiver or treated conservatively over the
realizations included in the analysis. The present comparison is
restricted to the additive-noise realization and to attenuator
realizations with pure Gaussian environmental states. Mixed
environmental states and other Gaussian channel constructions
compatible with the same receiver statistics were not considered.
