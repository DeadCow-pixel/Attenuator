...
In this paper, we have studied two Gaussian channels that model measurement asymmetry in the untrusted-noise CV-QKD -- the additive noise channel and the attenuator channel. 
Using the fact the action of the dual channel on the covariance matrix of the POVM of the ideal measurement must return the covariance matrix of the imperfect measurement (see Eq.~\eqref{eq:C-gen}) to ensure that the measured statistics are equivalent~\eqref{eq:channel-motiv}, we have derived the coefficients that connect the models to the parameters of the scheme. %These results were applied to the GMCS protocol. 
This work directly addresses the methodological limitations of our previous study~\cite{naumchik2025asymmetryeffectshomodyneheterodyne}, where only the additive noise channel was considered.

Our comparative analysis reveals that the choice of the channel model significantly affects the evaluated bounds for the secure key rate of the GMCS protocol (see Fig.~\ref{fig:h-comparison-CM} and~\ref{fig:dh-comparison-CM}). While in both cases asymmetry is detrimental, the attenuator channel yields less pessimistic bounds and significantly greater reach (maximum transmission distance) that the additive noise channel. For homodyne detection, attenuator channel simply scales both the effective channel transmission and the excess noise by a factor of $\sigma _{x}^{-1}$~\eqref{eq:tot_hom}.

For double-homodyne detection, an inherent ambiguity arises due to the dependence of the double homodyne POVM on the squeezing parameter, which we thoroughly discussed in Ref.~\cite{naumchik2025asymmetryeffectshomodyneheterodyne}. Consequently, for both models, the channel coefficients depend on the squeezing parameter~\eqref{eq:r1-r2}. Due to this dependency, the joint entropy, and therefore the Holevo information, depend on the squeezing parameter as well. Moreover, the dependence of the Holevo information on the squeezing parameter is drastically different for the two channel models (see Fig.~\ref{fig:Entropy}). For additive channel, Holevo information is convex in $r$, whereas for attenuator channel it is concave in $r$.% The curvature of this dependency is preserved for all parameters of the scheme. 
Due to this, the choices of both the channel model and the value of $r$ yield significantly different results for the case of double-homodyne detection.

Our concluding remarks concern the generalization of our results. \textcolor{red}{As shown in Appendix~\ref{sec:lossy-bs}}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{align}
    ...
    \end{pmatrix}=
    \begin{pmatrix}
        \sqrt{\tau_1}C & -S\sqrt{\tau_1}\\
        \sqrt{\tau_2}S & C\sqrt{\tau_2}
    \end{pmatrix}
    \begin{pmatrix}
        \hat{a}\\\hat{a}_L
    \end{pmatrix}+
...
\end{align}
Because the matrix $\vc{A}\hcnj{\vc{A}}$ representing the losses is diagonal, it is straightforward to see that to account for lossy beamsplitter in a homodyne setup it is sufficient to simply rescale the quantum efficiencies of detection: $\eta_{1,2}\to\sqrt{\tau_{1,2}}\eta_{1,2}$. The same argument would apply in the case of double homodyne detection.
...
\begin{align}
  ...
  &
    ...
    \approx
    (\eta_1\rho^2+\eta_2\tau^2) \left|\alpha_L\right|^2 \textcolor{red}{+\frac{\eta_1t\rho+\eta_2\tau r}{2}\left|\alpha_L\right| \avr{\hat{x}_\phi}},
  \\
  &
    \label{eq:mu_G}
    \mu_G=\eta_1|\alpha_1|^2-\eta_2|\alpha_2|^2
    \approx
    (\eta_1\rho^2+\eta_2\tau^2) \left|\alpha_L\right|^2
    +\frac{\eta_1t\rho-\eta_2\tau r}{2}\left|\alpha_L\right| \avr{\hat{x}_\phi}
\end{align}
