...
\begin{abstract}
    We study the security of continuous-variable quantum key distribution (CV-QKD) under asymmetric coherent detection within an untrusted-detector model. We analyze the attenuator channel and the additive-noise channel. For both homodyne and double-homodyne detection, we derive the corresponding channel parameters and compare the resulting Holevo information and asymptotic secret fraction.
  
  For the parameter sets considered, receiver asymmetry degrades the protocol performance for both channel models, but neither model gives uniformly more or less pessimistic security estimates. For homodyne detection, the attenuator model gives a lower secret fraction at higher transmittance but retains a positive secret fraction over a longer distance.

  In the double-homodyne case, the channel parameters and the Holevo information, and hence the resulting secret fraction, depend on the squeezing parameter $r$ that defines ideal measurements. %, whereas the Alice-Bob mutual information calculated from the calibrated receiver statistics does not.
...
In the limit of a strong local oscillator, these coherent measurement schemes function as Gaussian measurements, yielding a Gaussian probability distribution of outcomes for any input Gaussian state~\cite{Giedke:pra:2002,Fiurasek:pra:2007,Serafini:bk:2023}.
In an ideal, noiseless setup, homodyne detection implements projective measurements onto quadrature eigenstates, whereas double homodyne detection realizes a generalized measurement described by a rank-one positive operator-valued measure (POVM) onto coherent states~\cite{Serafini:bk:2023}.
However, a practical coherent receiver is characterized by a wide range of imperfections, such as non-unit photodetector efficiencies, electronic noise, beam-splitter imbalance, and mismatched responses of the detection branches~\cite{Pereira2021,Barchielli_2022,PRXQuantum.3.010341,hajomer2025finite,Bartlett:25}. Under these realistic conditions, the measurements retain their Gaussian statistics but are more generally described by Gaussian positive operator-valued measures (POVMs)~\cite{Ali1974, Ali2009, Serafini:bk:2023}.
% Measurement imperfections give rise to noisy POVMs~\cite{Ali1974, Ali2009, Serafini:bk:2023}.

%^ links
% more works abt homodyne noise

\textcolor{magenta}{SOMEWHERE in HERE should be historical lit rev ABOUT noisy coherent measurements}

%%channel rep
%To describe these imperfect measurements, the formalism of quantum channels is employed. 
Since quantum channels generally describe the evolution of open quantum systems, they provide a natural framework for modeling the physical degradation of information during the measurement process~\cite{Davies1970,HOLEVO1977273,Holevo1973Statistical, Ali1974, Ozawa1984}. Mathematically, a quantum channel $\mathcal{C}$ is a linear, completely positive, and trace-preserving (CPTP) map that transforms an input density matrix $\rho$ into an output state according to the Kraus representation:
\begin{equation}
  \mathcal{C}(\rho )=\sum _{k}E_{k}\rho E_{k}^{\dag },
\end{equation}
where the Kraus operators satisfy the completeness relation $sum_k E_k^\dagger E_k = \mathbb{I}$~\cite{Nielsen:bk:2010}. In this framework, the statistics of noisy measurements can be represented either by applying a CPTP channel to the incoming state prior to an ideal measurement, or, equivalently, by applying the dual map \(\mathcal{C}^{*}\) to the ideal POVM~\cite{Serafini:bk:2023}.
...
This channel representation is particularly powerful in CV-QKD because it treats the physical mechanisms responsible for non-unitary evolution -- such as optical losses and coupling to the environment -- on an equal footing, regardless of whether they occur during propagation (due to communication channel) or on the sender (Alice)/receiver (Bob) side.  Consequently, various channel models are available to capture different physical situations~\cite{Caruso2006}. The most prominent are the attenuator (beam-splitter) channel~\cite{laudenbach2018continuous}, which is used for modeling physical losses and thermal environments, and the additive-noise channel~\cite{naumchik2025asymmetryeffectshomodyneheterodyne}, which effectively captures electronic noise or phase jitter. With a suitable choice of parameters, these channel models reproduce the exact same conditional outcome distributions after a proper rescaling of the measurement data.

%the statistics are the same but the security bounds are not ~\cite{Staffieri_2026}
The choice of channel model directly dictates the resulting security bounds in CV-QKD, as different representations distribute quantum information to unobserved degrees of freedom in distinct ways (\textcolor{magenta}{or \emph{estimate intruder's (Eve) information differently}; \textbf{choose 1}}). This difference in estimaion is alike to security paradigm adopted for the calibrated receiver noise.
\emph{textcolor}{red}{In a trusted-detector model, the degrees of freedom associated with calibrated receiver noise are assumed to remain safely inside Bob's laboratory~\cite{Lodewyck:pra:2007,Usenko:entropy:2016,Laudenbach:aqt:2018,Laudenbach:aqt:2019,Namiki2018DetectionChannel}. In the untrusted-detector model these environmental modes are fully assigned to Eve. Under this untrusted noise paradigm, different mathematically equivalent channels yield identical calibrated statistics and mutual information, yet they generate different shared Alice-Bob states and different environmental outputs. This choice of channel model directly alters the resulting security bounds. In the untrusted-noise scenario, selecting an inappropriate or overly conservative channel model leads to vastly different values of the Holevo information, dictating a much more pessimistic performance and strictly limiting the key rate. More recent works further refine this dichotomy by considering intermediate levels of trust for loss and noise, separating Eve's knowledge of detector noise from her ability to control it~\cite{PirandolaTrustLevels2021,Pan:pra:2025,Zuo2026ThreatModels}. Since the purifications of a fixed state are related by an isometry on the purifying system~\cite{Nielsen:bk:2010}, applying alternative channels to the incoming state alters the effective Alice-Bob state prior to the ideal measurement, cementing the choice of channel representation as a fundamental physical assumption rather than a mere mathematical convenience.
...
To address this issue, the goal of this work is to quantify the fundamental discrepancy in security bounds that arises purely from the mathematical representation of receiver imperfections. We aim to show that even with identical raw data and mutual information, the choice of the quantum channel representation introduces non-trivial physical assumptions about Eve's coupling, which can drastically alter the final security guarantees of the system.

To this end, we construct and analyze an attenuator channel of the imperfect homodyne and double-homodyne POVMs and compare it with the additive-noise channel. The channel parameters are obtained by requiring the dual channel to map the ideal POVM onto the target noisy POVM. For homodyne detection, the environmental mode is chosen to be in the vacuum state, so the attenuator takes the form of a pure-loss channel. For double-homodyne detection, the generally anisotropic measurement noise is reproduced using a pure Gaussian environmental state with unequal quadrature variances.% The channels considered here, therefore, serve as a representation of a fixed noisy measurement rather than as alternatives to the physical communication link.
We compare both channels within the Gaussian-modulated coherent-state (GMCS) protocol~\cite{Grosshans:prl:2002} with reverse reconciliation in the asymptotic regime and evaluate the asymptotic secret fraction under collective Gaussian attacks~\cite{Navascues2006GaussianAttacks,GarciaPatron2006GaussianAttacks}. In the security model adopted here, receiver asymmetry is treated as untrusted; accordingly, the environmental output of the selected channel is assigned to Eve. %Finite-size effects, composable-security corrections, active detector attacks, and uncertainties in equipment calibration are outside the scope of the present work.

This paper is organized as follows. In Sec.~\ref{sec:POVMs}, we briefly discuss the formalism behind POVMs of imperfect homodyne and double homodyne measurement schemes.  In Sec.~\ref{sec:channels}, we model these noisy coherent detection schemes using the attenuator and additive-noise quantum channels.
...
The $Q$ symbol of the POVM describing noisy homodyne measurements (see Fig.~\ref{subfig:homodyne}) is
\begin{align}
...
\begin{align}
  ...
  &\Sigma_{G}\mapsto  X_{\mathcal{C}}^{-1}\Sigma_{G} \left({X_{\mathcal{C}}^{-1}}\right)^{\ind{T}}+X_{\mathcal{C}}^{-1}Y_{\mathcal{C}}\left({X_{\mathcal{C}}^{-1}}\right)^{\ind{T}}.
\end{align}
