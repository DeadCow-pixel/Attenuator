...
\usepackage{fancyhdr}% for footers and headers
\usepackage{geometry}
...
\definecolor{main-blue}{RGB}{68,117,161}
\renewcommand{\arraystretch}{1.5}

\year{2025} \volume{16} \nomer{1} \firstpage{0}
\shortcite{Naumchik A.S., et al.} %The first author

\title[Short title]{Asymmetry effects in untrusted noise securuty analysis} %Short and full titles

\author[A.\,S. Naumchik, R.\,K. Goncharov, A.\,D. Kiselev]{Anastasia S. Naumchik$^{1,a}$, Roman K. Goncharov$^{1,b}$, Alexei D. Kiselev$^{1,c}$} % Short and full author's names

\authorcopy{Naumchik A.S., Goncharov R.K., Kiselev A.D.}
\corrauthor{Anastasia S. Naumchik, asnaumchik@itmo.ru}

\address{$^1$ITMO University, St. Petersburg, Russia}

\email{$^a$asnaumchik@itmo.ru, $^b$goncharov@itmo.ru, $^c$adkiselev@itmo.ru}

\pacs{...} % PACS indexes if needed

\keywords{\textcolor{red}{energy; quantum.}}

\acknowledgements{\textcolor{red}{The work was financially supported by}}

\abstract{\textcolor{red}{Text of the Abstract}}
...
First, we shall use the attenuator channel $\mathcal{E}_{(\gamma)}$,
characterized by matrices~\cite{Serafini:bk:2023}
\begin{align}
    \label{eq:X,Y}
    X_{\mathcal{E}_{\gamma}}=\cos\theta_{\gamma}\mathbbm{1}, \quad Y_{\mathcal{E}_{\gamma}}=\sin^2\theta_{\gamma}\Sigma_{\ind{env}}^{(\gamma)}, \quad \gamma \in \{\ind{H}, \ind{DH}\},
\end{align}
where $\Sigma_{\ind{env}}^{(\gamma)}$ is the covariance matrix of the state decribing the enviroment. This channel acts on first and second moments in accordance with Eq.~\eqref{eq:X,Y-action}.
...
\begin{align}
  ...
  \tan^2\theta_{\ind{H}}\mathbbm{1}=\Sigma_{\mathcal{E}_{\ind{H}}}, \quad
    \Sigma_{\mathcal{E}_{\ind{H}}}\equiv R(\phi)\,\ind{diag}(\sigma_N,\sigma_N)\,{R}^{\ind{T}}(\phi).
\end{align}
...
We consider additive noise (classical mixing) channel~\cite{Serafini:bk:2023} $\Phi_{\gamma}$, characterized by matrices
\begin{align}
    \label{eq:X,Y-phi}
    X_{\Phi_{\gamma}}=\mathbbm{1}, \quad Y_{\Phi_{\gamma}}\geq0.
\end{align}
...
To find matrices $Y_{\Phi_{\gamma}}$ so that
\begin{align}
  \Phi_{\gamma}:\quad \Sigma_{\ind{id}}^{(\gamma)}\mapsto\Sigma_m^{(\gamma)},
\end{align}
we need to find the difference between matrices of noisy and ideal measurements, i.e.
\begin{align}
  Y_{\Phi_{\gamma}}=\Sigma_{m}^{(\gamma)}-\Sigma_{\ind{id}}^{(\gamma)}\equiv \Sigma_{N}^{(\gamma)},
\end{align}
...
\end{align}
The states pass the composite Gaussian channel $\mathcal{N}_{A\to B}=\mathcal{E}_{T_{\ind{ch}}}\circ\mathcal{E}_{N}$, $N=1+\xi_{\ind{ch}}/(1-T_{\ind{ch}})$, where $T_{\ind{ch}}$ is the channel transmittance and $\xi_{\ind{ch}}$ is the channel excess noise~\cite{laudenbach2018continuous}.
...
When the channel $C_{\gamma}\in\{\Phi_{\gamma}, \mathcal{E}_{\gamma}\}$ is under control of the eavesdropper, 
Eve holds a purification of the state
$\hat{\rho}_{\ind{AB}}^{\left(C_{\gamma}\right)}=\mathcal{I}_A\otimes C_{\gamma}(\hat{\rho}_{\ind{AB}})$ and the measurements performed by the receiver
...
\begin{align}
  \label{eq:chi}
  &
  \chi_{\ind{EB}} ^{\left(C_{\gamma}\right)}
    =
    S_{\ind{E}}-S_{\ind{E}|\ind{B}}=S_{\ind{AB}}^{\left(C_{\gamma}\right)}-S_{\ind{A}|\ind{B}}^{\left(C_{\gamma}\right)}, \quad C_{\gamma}\in\{\Phi_{\gamma}, \mathcal{E}_{\gamma}\}.
\end{align}
...
\begin{align}
  &
  \label{eq:S_AB-gamma}
  S_{\ind{AB}}^{\left(C_{\gamma}\right)}=g(\nu_1^{\left(C_{\gamma}\right)})+g(\nu_2^{\left(C_{\gamma}\right)}),
  \\
  &\notag
    g(\nu)=\frac{\nu+1}{2}\log \frac{\nu+1}{2}-
    \frac{\nu-1}{2}\log\frac{\nu-1}{2}
...
\nu_{1,2}^{\left(C_{\gamma}\right)}
    =\sqrt{
\Delta_{C_{\gamma}}/2\pm \sqrt{\Delta_{C_{\gamma}}^2/4-D_{C_{\gamma}}}
    },
    \quad
  D_{C_{\gamma}}=\det \Sigma_{\ind{AB}}^{\left(C_{\gamma}\right)},  
    \\
  &
  \label{eq:D-gamma}
  \Delta_{C_{\gamma}}=\det([\Sigma_{\ind{AB}}^{\left(C_{\gamma}\right)}]_{11})+\det([\Sigma_{\ind{AB}}^{\left(C_{\gamma}\right)}]_{22}) +
  2\det([\Sigma_{\ind{AB}}^{\left(C_{\gamma}\right)}]_{12}),
\end{align}
where $[\Sigma_{\ind{AB}}^{\left(C_{\gamma}\right)}]_{ii}$ ($[\Sigma_{\ind{AB}}^{\left(C_{\gamma}\right)}]_{ij}$) are diagonal (off-diagonal) blocks of $\Sigma_{\ind{AB}}^{\left(C_{\gamma}\right)}$.
...
\begin{align}
\label{eq:r}
K^{\left(C_{\gamma}\right)}=\beta I_{AB}^{\left(C_{\gamma}\right)}-\chi_{EB}^{\left(C_{\gamma}\right)},
\quad \gamma\in\{\ind{H},\ind{DH}\}, C_{\gamma}\in\{\Phi_{\gamma}, \mathcal{E}_{\gamma}\}.
\end{align}
...
\begin{align}
    ...
    \begin{pmatrix}
        V\mathbbm{1}&{c}\cos\theta^{(\mathcal{E}_{\ind{DH}})}\sigma_z\\
        c\cos\theta^{(\mathcal{E}_{\ind{DH}})}\sigma_z&
        B
    \end{pmatrix},\\
    \notag
    &B=\cos^2\theta^{(\mathcal{E}_{\ind{DH}})}\begin{pmatrix}
      {V_{\ind{B}}}+\delta_1-\frac{e^{2r}}{\cos^2\theta^{(\mathcal{E}_{\ind{DH}})}}&0\\
      0&{V_{\ind{B}}}+\delta_2-\frac{e^{-2r}}{\cos^2\theta^{(\mathcal{E}_{\ind{DH}})}}
    \end{pmatrix}.
    \end{align}
...
\bibliography{refs, quant, homodyne, math}

\textcolor{red}{$\uparrow$ refs may need manual input}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%55
\noindent\textcolor{main-blue}{\rule{\textwidth}{1.5pt}}
\begin{flushright}
	\emph{Submitted 1 November 2024; revised 1 December 2024; accepted 1 January 2025}
\end{flushright}

{ \setlength{\parindent}{0mm} \setlength{\parskip}{3mm}
\textit{Information about the authors:}

\textit{Anastasia S. Naumchik} -- ITMO University, Kronverkskiy, 49, St. Petersburg, 197101, Russia; ORCID 0009-0002-9376-5004; asnaumchik@itmo.ru

\textit{Roman K. Goncharov} -- ITMO University, Kronverkskiy, 49, St. Petersburg, 197101, Russia; ORCID 0000-0002-9081-8900; rkgoncharov@itmo.ru

\textit{Alexei D. Kiselev} -- ITMO University, Kronverkskiy, 49, St. Petersburg, 197101, Russia; ORCID 0000-0002-1023-3284; adkiselev@itmo.ru

\textit{Conflict of interest}: the authors declare no conflict of interests.
}
