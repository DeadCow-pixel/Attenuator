...
An important property is that the same measurement statistics can be described by different Gaussian channels $\mathcal{C}$, provided their dual maps $\mathcal{C}^{*}$ yield the same effective POVM $\hat\Pi_G^{(x)}=\mathcal{C}^{*}\hat\Pi_{\ind{id}}^{(x)}$. 
