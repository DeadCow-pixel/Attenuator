...
It is known that POVMs of noisy measurements can be introduced using the dual of a quantum channel acting on POVMs representing perfect (noiseless) measurements~\cite{Serafini:bk:2023}. 
From the $Q$ symbol~\eqref{eq:Pgood-w-def}, using formula for overlap of two Gaussian states~\cite{Serafini:bk:2023}, we deduce the following expression for the covariance matrix of noisy homodyne measurements
\begin{align}
    &\label{eq:Sigma-H-m}
...
\end{align}
where $R(\phi)=\begin{pmatrix}\cos\phi&-\sin\phi\\ \sin\phi& \cos\phi\end{pmatrix}$ is the rotation matrix; $\sigma_N$ is the excess noise variance that accounts for asymmetry effects is given by
\begin{align}
  &
  \label{eq:sgm_N}
     \sigma_N=\sigma_x-1.
...
\section{Gaussians channels}
In what follows, we shall use the attenuator quantum channel~\cite{Serafini:bk:2023},
characterized by matrices
\begin{align}
    \label{eq:X,Y}
    X^{(\gamma)}=\cos\theta^{(\gamma)}\mathbbm{1}, \quad Y^{(\gamma)}=\sin^2\theta^{(\gamma)}\Sigma_{\ind{aux}}^{(\gamma)},
\end{align}
\begin{align}
  \label{eq:E_gamma}
  &
    \mathcal{E}_N^{(\gamma)}:
    \hat{\Pi}_{\text{id}}\mapsto
    \hat{\Pi}_m^{(\gamma)}= \mathcal{E}_N^{*(\gamma)}(\hat{\Pi}_{\text{id}})=
    \notag\\
    &\frac{2}{\pi\cos\theta^{(\gamma)}\sin\theta^{(\gamma)}\sqrt{\det\Sigma_{\text{aux}}^{(\gamma)}}}
    \int_{\mathbb{C}} \mathrm{d}^2\beta\,
    \exp\left\{-2\mathbf{s}^T\Sigma_{\text{aux}}^{-1}\mathbf{s}\right\}
    \hat{D}(\cos\theta^{(\gamma)}\,\beta)\,
    \hat{\Pi}_{\text{id}}\,
    \hat{D}(-\cos\theta^{(\gamma)}\,\beta).
\end{align}
where
$\tcnj{\vc{s}}=(\cos\theta^{(\gamma)}\Re(\beta), \cos\theta^{(\gamma)}\Im(\beta))$,
$\dd^2\beta=\dd \beta_1\dd \beta_2$
and $\hat{D}(\beta)$ is the displacement operator given by
\begin{align}
    \label{eq:D-def}
\hat{D}(\beta) =
...
\begin{align}
    \label{eq:psi_ast}
    \mathcal{E}_N^{*(\gamma)}:\quad \Sigma_{\ind{id}}^{(\gamma)}\mapsto\Sigma_{m}^{(\gamma)}=\frac{1}{\cos^2\theta^{(\gamma)}}\Sigma_{\ind{id}}^{(\gamma)}+\tan^2\theta^{(\gamma)}\Sigma_{\ind{aux}}^{(\gamma)},
...
In untrusted noise scenario, where excess noise caused by asymmetry effects is assumed to be in control of Eve, channel that models the excess noise can be modeled in two equivalent ways, per Eq.~\eqref{eq:psi_ast} and Eq.~\eqref{eq:Phi_N}.

We consider local action of either attenuator channel $\mathcal{E}_N^{(\gamma)}$~\eqref{eq:psi_ast} or additive noise channel $\Psi^{(\gamma)}$~\eqref{eq:Phi_N}, on second mode of TMSVS shared by Alice and Bob~\eqref{eq:TMSVS-to-AB}. In both cases, mutual information is given by $I_{\ind{AB}}^{(\gamma)}$~\eqref{eq:I_H_res},~\eqref{eq:I_H_res}, as it is classical.
...
\begin{align}
    ...
    \Sigma_{\ind{AB}}\mapsto
     \Sigma_{\ind{AB}}^{(\gamma;\, \mathcal{E}_N)}
     =\begin{pmatrix}
        V\mathbbm{1}&c\cos\theta^{(\gamma)}\sigma_z\\
     c\cos\theta^{(\gamma)}\sigma_z&\cos^2\theta^{(\gamma)}
     \left(V_{\ind{B}}+\tan^2\theta^{(\gamma)}\Sigma_{\ind{aux}}^{(\gamma)}\right)
    \end{pmatrix},
\end{align}
\begin{align}
  &
...
When the channel $C\in\{\Phi_N^{(\gamma)}, \mathcal{E}_N^{(\gamma)}\}$ is under control of the eavesdropper, 
Eve holds a purification of the state
$\hat{\rho}_{\ind{AB}}^{(\gamma;\, C)}=\mathcal{I}_A\otimes C^{(\gamma)}(\hat{\rho}_{\ind{AB}})$ and the measurements performed by the receiver
...
\begin{align}
  ...
  \chi_{\ind{EB}} ^{(\gamma;\, C)}
    =
    S_{\ind{E}}-S_{\ind{E}|\ind{B}}=S_{\ind{AB}}^{(\gamma;\, C)}-S_{\ind{A}|\ind{B}}^{(\gamma;\, C)}, \quad C\in\{\Phi_N^{(\gamma)}, \mathcal{E}_N^{(\gamma)}\}.
\end{align}
...
\nu_{1,2}^{(\gamma;\, C)}
    =\sqrt{
\Delta_{\gamma;\, C}/2\pm \sqrt{\Delta_{\gamma;\, C}^2/4-D_{\gamma;\, C}}
    },
    \quad
  D_{\gamma;\, C}=\det \Sigma_{\ind{AB}}^{(\gamma;\, C)},  
    \\
  &
  \label{eq:D-gamma}
  \Delta_{\gamma;\, C}=\det([\Sigma_{\ind{AB}}^{(\gamma;\, C)}]_{11})+\det([\Sigma_{\ind{AB}}^{(\gamma;\, C)}]_{22}) +
  2\det([\Sigma_{\ind{AB}}^{(\gamma;\, C)}]_{12}),
\end{align}
where $[\Sigma_{\ind{AB}}^{(\gamma;\, C)}]_{ii}$ ($[\Sigma_{\ind{AB}}^{(\gamma;\, C)}]_{ij}$) are diagonal (off-diagonal) blocks of $\Sigma_{\ind{AB}}^{(\gamma;\, C)}$.
...
\begin{align}
\label{eq:r}
K^{(\gamma;\, C)}=\beta I_{AB}^{(\gamma;\, C)}-\chi_{EB}^{(\gamma;\, C)},
\quad \gamma\in\{\ind{H},\ind{DH}\}, C\in\{\Phi_N^{(\gamma)}, \mathcal{E}_N^{(\gamma)}\}.
\end{align}
...
\begin{align}
    ...
    &\Sigma_{\ind{AB}}^{(\ind{H};\, \mathcal{E}_N)}=
    \begin{pmatrix}
...
Symplectic eigenvalues of $\Sigma_{\ind{AB}}^{(\ind{H};\, \mathcal{E}_N)}$, $\nu_{1,2}^{(\ind{H};\,\mathcal{E}_N)}$, can be computed using formulas~\cite{Weed:rmp:2012}:
\begin{align}
    \label{eq:lambda12-h}
    \nu_{1,2}^{(\ind{H};\, \mathcal{E}_N)}=\frac{z\pm[\tilde{V}_{\ind{B}}-V]}2,\quad z=\sqrt{\left(V+\tilde{V}_{\ind{B}}\right)^2-4{\tilde{c}^2}},
\end{align}
and then substitued in Eq.~\eqref{eq:S_AB-gamma} to calculate joint entropy $S_{\ind{AB}}^{(\ind{H};\,\mathcal{E}_N)}$.
...
\includegraphics[]{pics/double-homodyne/Sab.pdf}
 \caption[]{Joint entropy, $S_{\ind{AB}}^{(\ind{DH}; C)}$, as a function of the squeezing parameter $r$ at
...
\begin{align}
    &\Sigma_{\ind{AB}}^{(\ind{DH};\, \mathcal{E}_N)}=
    \notag\\
    &
    \begin{pmatrix}
        V\mathbbm{1}&{c}\cos\theta^{(\ind{DH};\, \mathcal{E}_N)}\sigma_z\\
        c\cos\theta^{(\ind{DH};\, \mathcal{E}_N)}\sigma_z&
        \begin{matrix}
            \cos^2\theta^{(\ind{DH};\, \mathcal{E}_N)}\left({V_{\ind{B}}}+\delta_1-\frac{e^{2r}}{\cos^2\theta^{(\ind{DH};\, \mathcal{E}_N)}}\right)&0\\
            0&\cos^2\theta^{(\ind{DH};\, \mathcal{E}_N)}\left({V_{\ind{B}}}+\delta_2-\frac{e^{-2r}}{\cos^2\theta^{(\ind{DH};\, \mathcal{E}_N)}}\right)
        \end{matrix}
    \end{pmatrix}.
    \end{align}
...
Fig.~\ref{fig:Entropy} illustrates dependence of the joint entropy on the squeezing parameter, for both additive noise and attenuator channels. Referring to Fig.~\ref{fig:Entropy}, in the limiting case of ideal double homodyne with $\delta_1=\delta_2=1$, only one value of $r=0$ is allowed (see Ref.~\cite{naumchik2025asymmetryeffectshomodyneheterodyne}), with matching joint entropies for both channels (black dot on plot).

When $\delta_1=\delta_2>1$, dependence of joint entropy becomes symmetrical around $0$ for both channels. For additive noise channel, $r=0$ is the point of maximal joint entropy, while for attenuator channel $r=0$ corresponds to minimal joint entropy. Moreover, while joint entropy is unidefined at $r=r_{2,1}$ (see Eq.~\eqref{eq:cos-theta-dh}), the values tend to the same values of joint entropy in the case of additive noise channel, as signified by color-matched white-filled dots in Fig.~\ref{fig:Entropy}.

In asymmetrical case where $\delta_1\neq\delta_2$, joint entropies for both channel models become asymmetrical relative to their extrema, resulting in unequal joint entropies at $r=r_{2,1}$. For additive noise channel, joint entropy has a distintive maxima, while for attenuator channel joint entropy has a distintive minima.
